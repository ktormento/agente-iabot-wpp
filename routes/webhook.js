...
router.post("/", async (req, res) => {
  ...
  if (body.object) {
    ...
    if (message) {
        ...
        const profileDetected = scoreMatch(text, profile);
        const intentDetected = scoreMatch(text, intent);

        const profile = profileDetected || user?.profile;
        const intent = intentDetected || user?.lastIntent;

        await updateUser(from, profile, intent);
